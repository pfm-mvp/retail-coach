...
try:
    from shop_mapping import SHOP_ID_TO_NAME  # exact naam zoals jij gebruikt
except Exception:
    SHOP_ID_TO_NAME = {}
...
with st.container():
    ...
    with c1:
        if SHOP_ID_TO_NAME:
            ...
            shop_name = st.selectbox("Selecteer een winkel", [o[0] for o in opts])
            shop_ids = [sid for (name, sid) in opts if name == shop_name]
        else:
            shop_text = st.text_input("Shop IDs (komma‑gescheiden)", value="")
            shop_ids = [int(x.strip()) for x in shop_text.split(",") if x.strip().isdigit()]
    with c2:
        analyze = st.button("🔍 Analyseer", type="primary", use_container_width=True)
...
if analyze:
    if not shop_ids:
        st.info("Selecteer of vul 1 of meer shops in.")
        st.stop()
    ...
    st.info("Selecteer een winkel en klik **Analyseer**.")
